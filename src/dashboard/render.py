...
def _overview_bullets_metrics_only(data: dict[str, Any]) -> str:
    """基于新闻摘要的速览（不含乖离、均线、量比等规则项）。"""
    digest = data.get("news_digest")
    sentiment = _first_line(str(digest) if digest else None) or "暂无联网摘要。"

    bar_date = data.get("date") or "-"
    latest = _first_line(str(digest) if digest else None, 160)
    if not latest:
        latest = "无摘要，请查看邮件底部「新闻摘要」区块。"
...
def _render_stock_card_metrics_only(data: dict[str, Any]) -> str:
    code = str(data.get("code", ""))
    mk = data.get("market") or MARKET_CN_SH
    name = data.get("name")
    ...
    overview = _overview_bullets_metrics_only(data)
...
<div class="stock-card">
  ...
  <div class="section-title">📈 当日行情</div>
  <p class="time-hint" style="margin:0 0 8px 0;">⏰ 基准行情日：<strong>{_e(bar_date)}</strong>（邮件生成时可能已跨日，请以实盘为准）</p>
  ...
  <table class="data-table sub-row">
    <tbody>
      <tr>
        <th>行情来源</th><td colspan="4">多源聚合（AkShare / Stooq / yfinance 等）</td>
      </tr>
    </tbody>
  </table>
</div>
...
def render_dashboard_html(
    ...
    *,
    ...
    cards = [_render_stock_card_metrics_only(m) for m in stock_metrics]
    stocks_html = "\n".join(cards) if cards else '<div class="card">暂无个股行情数据</div>'
...
<body>
  <div class="container">
    <h1>自选股 · 每日行情与新闻简报</h1>
    <div class="card">
      <strong>说明：</strong> {_e(llm_result.get("market_view", "本报告为行情与新闻汇总。"))}
      <br/>
